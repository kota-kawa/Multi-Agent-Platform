...
## Overview
- Flask 3 app (`app.py`) exposes the SPA, multi-agent orchestrator, and REST/SSE bridges to FAQ_Gemini, Browser Agent, and IoT Agent services.
...
## Key Modules & Files
- `app.py`: env bootstrapper, config constants, HTTP routes, LangGraph orchestrator, Browser/IoT proxy helpers, chat-history + memory endpoints.
...
- `Dockerfile` + `docker-compose.yml`: container build/run (port 5050) and network wiring for FAQ_Gemini + Browser Agent services.
...
- `_append_to_chat_history` maintains `chat_history.json`, appends every user/assistant turn, and asynchronously calls `_send_recent_history_to_agents` every five entries to sync FAQ_Gemini, Browser Agent, and IoT Agent.
...
- `.env` is auto-loaded at startup (`_load_env_file`). Required keys include `OPENAI_API_KEY` plus optional overrides such as:
  - `ORCHESTRATOR_MODEL`, `ORCHESTRATOR_MAX_TASKS`
  - `FAQ_GEMINI_API_BASE`, `FAQ_GEMINI_TIMEOUT`
...
- `docker compose up --build web` binds to port 5050 and injects service URLs for FAQ_Gemini/Browser Agent. Ensure the shared `multi_agent_platform_net` exists or set `MULTI_AGENT_NETWORK`.
...
- FAQ_Gemini proxies: `/rag_answer`, `/conversation_history`, `/conversation_summary`, `/reset_history`—all call `_call_gemini` with per-request logging/error handling.
